...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('chord-tones-updated', function(e) {
        setTimeout(validateChordNotes, 500);
    });
 ...
 */
function validateChordNotes(jsonData) {
    
    // Initialize the result object that we'll return at the end
    const result = { 
        mismatchCount: 0,
        expectedCount: 0,
        actualCount: 0,
    ...
    // First, check if we have valid data already provided
    if (jsonData && typeof jsonData === 'object') {
        chordData = jsonData;
    } 
    // Otherwise, try to safely get the data from the DOM
    else {
        try {
            // Check if dataset exists and has chordJson property
            if (chordDataElement.dataset && chordDataElement.dataset.chordJson) {
                ...
                if (jsonStr && (jsonStr.startsWith('{') || jsonStr.startsWith('['))) {
                    ...
                    console.warn("Invalid JSON string format in chord-data element");
                    
                    // Try using window.voicing_data as backup
                    if (window.voicing_data && typeof window.voicing_data === 'object') {
                        chordData = window.voicing_data;
                    } else {
                        return { mismatchCount: 0 };
                    }
                }
            } else {
                // If there's no chordJson in dataset, check if there's text content
                if (chordDataElement.textContent && chordDataElement.textContent.trim()) {
                    const textContent = chordDataElement.textContent.trim();
                    if (textContent.startsWith('{') || textContent.startsWith('[')) {
                        chordData = JSON.parse(textContent);
                    } else {
                        console.warn("Invalid JSON format in chord-data element text content");
                    ...
                    console.warn("No chord data available in element");
            ...
            console.error("Error accessing or parsing chord data:", error);
            
            // Try using window.voicing_data as backup in case of parsing error
            if (window.voicing_data && typeof window.voicing_data === 'object') {
                chordData = window.voicing_data;
            } else {
                return { mismatchCount: 0 };
    ...
    // If we still don't have valid data, return early
    if (!chordData) {
        console.warn("Could not get valid chord data");
        return { mismatchCount: 0 };
    }
    ...
    document.querySelectorAll('.note.active').forEach(note => {
        ...
        if (tone) {
            tone.classList.remove('active', 'root');
            tone.src = '/static/media/yellow_circle.svg';
        }
    });
    
    // Get position and range from URL
    const urlParams = new URLSearchParams(window.location.search);
    let position = urlParams.get('position_select') || 'Root Position';
    let range = urlParams.get('note_range') || urlParams.get('range') || 'e - g';
    
    // Normalize position names - handle "First Root Position" vs "Root Position" confusion
    if (position.includes("First Root") || position.includes("First Basic")) {
        position = "Root Position";
    }
    
    // Normalize "Root Position" vs "Basic Position" equivalence
    const lookupPosition = position === 'Root Position' ? 'Basic Position' : position;
    
    
    // Get position data - check both the actual position name and normalized version
    let positionData;
    if (window.voicing_data && voicing_data[range]) {
        // Try with the original position name first
        if (voicing_data[range][position]) {
            positionData = voicing_data[range][position];
        }
        // Try with the lookup position name (Basic Position instead of Root Position)
        else if (voicing_data[range][lookupPosition]) {
            positionData = voicing_data[range][lookupPosition];
        }
        // If neither worked, look for any available position data
        else {
            console.warn(`Position data not found for ${position} or ${lookupPosition}, trying fallbacks`);
            
            // Try other standard positions
            const possiblePositions = ['Root Position', 'Basic Position', 'First Inversion', 'Second Inversion'];
            
            for (const pos of possiblePositions) {
                if (voicing_data[range][pos]) {
                    positionData = voicing_data[range][pos];
                    position = pos; // Update position for consistent UI
                    break;
                }
            }
            
            // If still not found, just use the first available position
            if (!positionData) {
                const availablePositions = Object.keys(voicing_data[range])
                    .filter(key => key && typeof voicing_data[range][key] === 'object');
                
                if (availablePositions.length > 0) {
                    const firstAvailable = availablePositions[0];
                    positionData = voicing_data[range][firstAvailable];
                    position = firstAvailable; // Update position for consistent UI
                } else {
                    console.error("No valid position data found in voicing_data");
                    return { 
                        mismatchCount: 0,
        ...
        console.error("Missing voicing data for", range);
        return { 
            mismatchCount: 0,
            error: "Missing voicing data" 
        };
    }
    ...
    if (!positionData) {
        ...
        return { 
            mismatchCount: 0,
            error: "Could not find position data" 
        };
    }
    ...
    // Check if notes are already active
    const initialActiveNotes = document.querySelectorAll('.note.active').length;
    
    // Track activated notes
    let activatedNotes = 0;
    let rootNoteSet = false;
    
    // Track a static variable to prevent recursive activation
    if (!window._activationAttemptCount) {
        window._activationAttemptCount = 0;
    }

    // If no notes are active yet, let's attempt to activate them with getTonesFromDataChords
    if (initialActiveNotes === 0 && typeof getTonesFromDataChords === 'function' && window._activationAttemptCount < 2) {
        window._activationAttemptCount++;
        
        // Use getTonesFromDataChords to activate notes
        getTonesFromDataChords(position, range);
        
        // Give time for the notes to be activated, then check the result
        setTimeout(() => {
            const newActiveNotes = document.querySelectorAll('.note.active').length;
            
            // Reset the counter after a delay to avoid being stuck forever
            setTimeout(() => {
                window._activationAttemptCount = 0;
            }, 2000);
            
            // Only continue validation if we successfully activated notes
            if (newActiveNotes > 0) {
                validateChordNotes();
            } else {
            }
        ...
        return {
            mismatchCount: 0,
            message: "Activating notes first, will validate after"
        };
    } else if (window._activationAttemptCount >= 2) {
        window._activationAttemptCount = 0;
        
        return {
            mismatchCount: 0,
            message: "Maximum activation attempts reached"
        };
    }
    ...
    // Ensure we have exactly one root note
    if (!rootNoteSet) {
    ...
    document.dispatchEvent(new CustomEvent('chord-tones-updated', {
        detail: { position, range, success: true, source: 'validation' }
    }));
    
    // Return a result object with useful data
    return {
        mismatchCount: 0, // No mismatches in this implementation
        expectedCount: Object.keys(positionData).length,
        actualCount: activatedNotes,
        success: true
    };
}
...
function updateValidationPanel() {
    ...
    const expectedCount = chordType.toLowerCase() === 'major' ? 3 : 3; // Default to 3 for now
    
    // Create validation summary
    let summary = `<h3>Validation Results</h3>`;
    summary += `<p>Chord Type: ${chordType}</p>`;
    summary += `<p>Active Notes: ${activeCount} (Expected: ${expectedCount})</p>`;
    
    // Check root notes
    const rootNotes = document.querySelectorAll('img.tone.active.root');
    summary += `<p>Root Notes: ${rootNotes.length} (Expected: 1)</p>`;
    
    // List active notes
    let activeNotesList = '<h4>Active Notes:</h4><ul>';
    activeNotes.forEach(note => {
        ...
        const noteClass = noteClasses.find(cls => {
            const baseNote = cls.replace(/[0-9]/g, '');
            return ['a', 'ab', 'as', 'b', 'bb', 'c', 'cs', 'db', 'd', 'ds', 'eb', 'e', 'f', 'fs', 'gb', 'g', 'gs'].includes(baseNote);
        });
        
        const isRoot = note.querySelector('img.tone.root') !== null;
        activeNotesList += `<li>${noteClass || 'unknown'} on ${stringName}${isRoot ? ' (Root)' : ''}</li>`;
    });
    activeNotesList += '</ul>';
    
    // Update panel content
    panel.innerHTML = summary + activeNotesList;
    
    // Add warning if counts don't match
    if (activeCount !== expectedCount || rootNotes.length !== 1) {
        panel.innerHTML += `
            <div style="color: red; margin-top: 10px;">
                <strong>Warning:</strong> Validation issues detected:
                <ul>
                    ${activeCount !== expectedCount ? `<li>Incorrect number of active notes (${activeCount} vs ${expectedCount})</li>` : ''}
                    ${rootNotes.length !== 1 ? `<li>Incorrect number of root notes (${rootNotes.length} vs 1)</li>` : ''}
                </ul>
            </div>
        `;
    }
...
function attemptAutoFix() {
    ...
    if (document.querySelectorAll('.note.active').length > 6) {
        
        // Do a complete reset and rebuild
        
        // Reset all notes to a clean state
        ...
        // Use direct voicing data approach
        fixFromVoicingData(position, range);
    } else {
        // Quick fix for partial activation issues without full reset
        if (results.mismatchCount <= 3) {
            ...
            if (partialResults.mismatchCount === 0) {
                return;
            }
        }
        
        // If quick fixes didn't work, do a complete reset and rebuild
        ...
        // Try to intelligently select the right notes
